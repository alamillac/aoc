...
module IDRange = struct
  ...
  let is_invalid_id_v1 (id : int) : bool =
  ...
  let split_id (id : string) (partition : int) : string list =
    let num_digits = String.length id in
    let part_size = num_digits / partition in
    List.init partition (fun i ->
        let start = i * part_size in
        let end_ = start + part_size in
        String.sub id start (end_ - start))

  let is_repeated (id : string) (partition : int) : bool =
    let num_digits = String.length id in
    if num_digits mod partition = 0 then
      let id_splits = split_id id partition in
      let first_split = List.hd id_splits in
      List.for_all (fun s -> s = first_split) id_splits
    else false

  let is_invalid_id_v2 (id : int) : bool =
    ...
    let str_id = Int.to_string id in
    let num_digits = String.length str_id in
    let partitions = Seq.ints 2 |> Seq.take (num_digits - 1) |> List.of_seq in
    List.exists (is_repeated str_id) partitions

  let find_invalid_ids fn (idr : t) : int list =
    let s = seq idr in
    Seq.filter fn s |> List.of_seq
end
...
let part1 () =
  ...
  let find_invalid_ids = IDRange.find_invalid_ids IDRange.is_invalid_id_v1 in
  let invalid_ids = List.map find_invalid_ids id_ranges |> List.flatten in
  let sum_invalid_ids = List.fold_left ( + ) 0 invalid_ids in
  Printf.printf "Invalid ids: (%d)\n" sum_invalid_ids

let part2 () =
  let id_ranges = Util.read_lines file parse_line in
  let find_invalid_ids = IDRange.find_invalid_ids IDRange.is_invalid_id_v2 in
  let invalid_ids = List.map find_invalid_ids id_ranges |> List.flatten in
  let sum_invalid_ids = List.fold_left ( + ) 0 invalid_ids in
  Printf.printf "Invalid ids: (%d)\n" sum_invalid_ids
